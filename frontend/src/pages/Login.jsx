...
export default function Login({ setUser }) {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      if (userObj.role === "admin") navigate("/admin/users");
...
const handleLogin = async (e) => {
  ...
  try {
    ...
    const loggedInUser = {
      email: payload.email,
      role: payload.role,
      id: payload.id // if you have a user ID in the token
    };

    // Save user in localStorage for persistent login
    localStorage.setItem("user", JSON.stringify(loggedInUser));

    // Update the App.js user state
